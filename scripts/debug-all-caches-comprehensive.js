...
class CacheDebugger {
    ...
    async measureCacheSizes() {
        ...
        try {
            ...
            if (fs.existsSync(npmCache)) {
                sizes.npm = execSync(`du -sh "${npmCache}" | cut -f1`, { encoding: 'utf8' }).trim();
            }
        ...
        try {
            ...
            if (fs.existsSync(pixiCache)) {
                sizes.pixi = execSync(`du -sh "${pixiCache}" | cut -f1`, { encoding: 'utf8' }).trim();
            } else {
                sizes.pixi = 'empty/missing';
            }
        ...
        try {
            ...
            if (fs.existsSync(miseCache)) {
                sizes.mise = execSync(`du -sh "${miseCache}" | cut -f1`, { encoding: 'utf8' }).trim();
            } else {
        ...
        try {
            // Node modules count
            const nodeModulesCount = execSync(`find "${DEVELOPER_DIR}" -name "node_modules" -type d | wc -l`, { encoding: 'utf8' }).trim();
            sizes.nodeModules = parseInt(nodeModulesCount);
    ...
    async clearPixiCache() {
        this.log('Clearing Pixi cache...');
        try {
            // Remove pixi cache directory
            const pixiCache = '/Users/daniellynch/Library/Caches/rattler/cache';
            if (fs.existsSync(pixiCache)) {
                execSync(`rm -rf "${pixiCache}"`, { stdio: 'inherit' });
    ...
    async clearMiseCache() {
        ...
        try {
            execSync('mise cache clear', { stdio: 'inherit' });
            ...
            try {
                const miseCache = '/Users/daniellynch/.cache/mise';
                if (fs.existsSync(miseCache)) {
                    execSync(`rm -rf "${miseCache}"`, { stdio: 'inherit' });
    ...
    async fixBrokenSymlinks() {
        ...
        try {
            ...
            for (const link of brokenLinks) {
                if (link) {
                    execSync(`rm -f "${link}"`, { stdio: 'inherit' });
    ...
    async consolidatePythonEnvironments() {
        this.log('Consolidating Python environments...');
        try {
            // Remove orphaned venv
            const orphanedVenv = path.join(DEVELOPER_DIR, '.venv');
            if (fs.existsSync(orphanedVenv)) {
                execSync(`rm -rf "${orphanedVenv}"`, { stdio: 'inherit' });
                this.success('Removed orphaned .venv directory');
            }

            // Ensure pixi environment is the primary one
            process.chdir(DEVELOPER_DIR);
            execSync('pixi install --frozen', { stdio: 'inherit' });
            this.success('Pixi environment verified and frozen');

        } catch (error) {
            this.error('Failed to consolidate Python environments', error);
        }
    }
    ...
    async clearPythonCache() {
        ...
        try {
            ...
            execSync(`find "${DEVELOPER_DIR}" -name "__pycache__" -type d -exec rm -rf {} + 2>/dev/null || true`, { stdio: 'inherit' });

            // Remove .pyc files
            execSync(`find "${DEVELOPER_DIR}" -name "*.pyc" -delete 2>/dev/null || true`, { stdio: 'inherit' });

            // Remove .pyo files
            execSync(`find "${DEVELOPER_DIR}" -name "*.pyo" -delete 2>/dev/null || true`, { stdio: 'inherit' });
    ...
    async run() {
        ...
        await this.consolidatePythonEnvironments();
