...
class LooseFilesAuditor:
    ...
    def __init__(self):
        ...
        # #region agent log H_AUDIT_1
        with open('${DEVELOPER_DIR:-${DEVELOPER_DIR:-$HOME/Developer}}/.cursor/debug.log', 'a') as f:
            f.write(json.dumps({
                "id": f"log_{int(time.time()*1000)}_{'audit_1_' + str(hash(str(time.time())))[:8]}",
                "timestamp": int(time.time()*1000),
                "location": "audit_loose_files.py:LooseFilesAuditor:__init__",
                "message": "Starting loose files audit - enforcing Cursor IDE rules",
                "data": {"user_root": str(self.user_root), "cursor_rules": str(self.cursor_rules_dir)},
    ...
    def log_audit(self, message: str, data: Dict[str, Any], hypothesis_id: str):
        """Audit logging with instrumentation"""
        log_entry = {
            "id": f"log_{int(time.time()*1000)}_{str(hash(message + str(time.time())))[:8]}",
            "timestamp": int(time.time()*1000),
            "location": f"audit_loose_files.py:{self.__class__.__name__}:{message.replace(' ', '_')}",
            "message": message,
            "data": data,
            "sessionId": "debug-audit-loose-files",
            "runId": "run-1",
        ...
        try:
            for item in directory.iterdir():
                if item.is_file():
                    ...
                    if item.name.startswith('.') or item.suffix in ['.sh', '.md', '.json', '.log', '.txt', '.py']:
    ...
    def generate_reorganization_plan(self, audit_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Plan file moves
        for file_info in audit_results["loose_files"]:
        ...
        required_dirs = ['docs', 'testing', 'infra', 'data', 'api', 'graphql', 'federation']
        for audit_result in audit_results.values():
            if isinstance(audit_result, dict) and "missing_directories" in audit_result:
                for missing_dir in audit_result["missing_directories"]:
                    if missing_dir not in plan["directories_to_create"]:
                        plan["directories_to_create"].append(missing_dir)
    ...
    def run_comprehensive_audit(self) -> Dict[str, Any]:
        ...
        self.log_audit("Starting comprehensive audit", {}, "H_AUDIT_2")
        ...
        self.log_audit("Comprehensive audit completed", {
            "total_violations": total_violations,
            "actions_required": len(audit_results["reorganization_plan"]["actions_required"])
        }, "H_AUDIT_2")
    ...
    def display_audit_results(self, results: Dict[str, Any]):
        ...
        print("📁 LOOSE FILES IN USER ROOT:")
        ...
        print("
📁 LOOSE FILES IN DEVELOPER DIR:"        dev_loose = results["developer_audit"]["loose_files"]
        ...
        print("
📋 CURSOR RULES COMPLIANCE:"        rules = results["cursor_rules"]
        ...
        print("
🔗 GIT HOOKS & EVENT-DRIVEN ARCHITECTURE:"        hooks = results["git_hooks"]
        ...
        print("
🛠️  REORGANIZATION PLAN:"        plan = results["reorganization_plan"]
        ...
        print("
📊 SUMMARY:"        print(f"   Total violations: {total_violations}")
        print(f"   Actions needed: {len(plan['actions_required'])}")

        print("
💾 Detailed report saved to:"        print(f"   {self.audit_report}")
