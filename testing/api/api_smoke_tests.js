...
function runSmokeTest(service, command, expectedOutput = '', timeout = 10000) {
  try {
    // CONSOLE_LOG_VIOLATION: // CONSOLE_LOG_VIOLATION: console.log(`🔍 Testing ${service}...`);
    ...
    // CONSOLE_LOG_VIOLATION: // CONSOLE_LOG_VIOLATION: console.log(`${service}: ${success ? '✅ PASS' : '❌ FAIL'}`);
    if (!success && result) // CONSOLE_LOG_VIOLATION: // CONSOLE_LOG_VIOLATION: console.log(`Output: ${result.trim()}`);
    return success;
  } catch (error) {
    // CONSOLE_LOG_VIOLATION: // CONSOLE_LOG_VIOLATION: console.log(`${service}: ❌ FAIL - ${error.message.split('\n')[0]}`);
    return false;
  }
}

function testGraphQL() {
  // CONSOLE_LOG_VIOLATION: // CONSOLE_LOG_VIOLATION: console.log('🔍 Testing GraphQL infrastructure...');
  ...
  try {
    ...
    // CONSOLE_LOG_VIOLATION: // CONSOLE_LOG_VIOLATION: console.log(`GraphQL Libraries: ${hasGraphQL ? '✅ INSTALLED' : '❌ MISSING'}`);
    return hasGraphQL;
  } catch (error) {
    // CONSOLE_LOG_VIOLATION: // CONSOLE_LOG_VIOLATION: console.log('GraphQL Libraries: ❌ ERROR');
    return false;
  }
}

async function runComprehensiveSmokeTests() {
  // CONSOLE_LOG_VIOLATION: // CONSOLE_LOG_VIOLATION: console.log('🚀 COMPREHENSIVE API SMOKE TESTS ACROSS ALL MCP SERVERS\n');
  ...
  // CONSOLE_LOG_VIOLATION: // CONSOLE_LOG_VIOLATION: console.log(`\n📊 COMPREHENSIVE SMOKE TEST RESULTS:`);
  // CONSOLE_LOG_VIOLATION: // CONSOLE_LOG_VIOLATION: console.log(`Services Tested: ${total}`);
  // CONSOLE_LOG_VIOLATION: // CONSOLE_LOG_VIOLATION: console.log(`Services Operational: ${passed}`);
  // CONSOLE_LOG_VIOLATION: // CONSOLE_LOG_VIOLATION: console.log(`Success Rate: ${Math.round((passed/total) * 100)}%`);
  
  if (passed >= total * 0.8) {
    // CONSOLE_LOG_VIOLATION: // CONSOLE_LOG_VIOLATION: console.log('🎉 SYSTEM HEALTH: EXCELLENT (80%+ operational)');
  } else if (passed >= total * 0.6) {
    // CONSOLE_LOG_VIOLATION: // CONSOLE_LOG_VIOLATION: console.log('⚠️ SYSTEM HEALTH: GOOD (60-79% operational)');
  } else {
    // CONSOLE_LOG_VIOLATION: // CONSOLE_LOG_VIOLATION: console.log('❌ SYSTEM HEALTH: NEEDS ATTENTION (<60% operational)');
  }
